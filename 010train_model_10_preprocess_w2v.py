...
X = pd.read_csv("./out/007X_with_entity.txt", sep='\t', header=None,
                encoding='ISO-8859-1')  #######
_025POS_transform_to_unite=pd.read_csv("./in/025POS_transform_to_unite_preprocess.txt",sep="\t",header=None,encoding="utf-8")
y = np.load('./out/007X_with_entity.npy')  ####

f_svm = open("./out/results/010svm_10_preprocess_w2v.txt", 'w')  ###################################################
f_LogisticR = open("./out/results/010LogisticR_10_preprocess_w2v.txt", 'w')  ###################################################
f_RandomF = open('./out/results/010RandomF_10_preprocess_w2v.txt', 'w')  ###################################################
f_xgboost = open('./out/results/010xgboost_10_preprocess_w2v.txt', 'w')  ###################################################)
f_lightGBM = open('./out/results/010lightGBM_10_preprocess_w2v.txt', 'w')  ###################################################)
