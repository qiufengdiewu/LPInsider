...
# 计算词向量
# 包括计算对应的位置特征
def get_sent_vec(size, npLength, sent, model, model_train, lncRNA,protein):
    vec = []
    ...
    vec = np.concatenate((vec, lncRNA_matrix, protein_matrix), axis=0)
    return vec
...
f_svm = open("./out/results/010svm_10_preprocess_w2v_stanfordparser_location.txt", 'w')  ###################################################
f_LogisticR = open("./out/results/010LogisticR_10_preprocess_w2v_stanfordparser_location.txt", 'w')  ###################################################
f_RandomF = open('./out/results/010RandomF_10_preprocess_w2v_stanfordparser_location.txt', 'w')  ###################################################
f_xgboost = open('./out/results/010xgboost_10_preprocess_w2v_stanfordparser_location.txt', 'w')  ###################################################)
f_lightGBM = open('./out/results/010lightGBM_10_preprocess_w2v_stanfordparser_location.txt', 'w')  ###################################################)
...
def train(X, y, count):
    ...
    #model = gensim.models.word2vec.Word2Vec.load(word2vec_path_train)#测试用
    for c in range(10):###################
        sentX = []
        length = 0
        ...
        for i in range(len(sentX)):
            sent=sentX[i]
            XX.append([get_sent_vec(200, length, sent, model, model_train,X[0][i],X[1][i])])
